...
public final class Tagged<S, B> implements Monad<B, Tagged<S, ?>>, Costrong<S, B, Tagged> {
    private final B b;
    ...
    public <Z> Tagged<Z, B> diMapL(Function<? super Z, ? extends S> fn) {
        return (Tagged<Z, B>) Costrong.super.<Z>diMapL(fn);
    }

    @Override
    public <C> Tagged<S, C> diMapR(Function<? super B, ? extends C> fn) {
        return (Tagged<S, C>) Costrong.super.<C>diMapR(fn);
    }

    @Override
    public <Z> Tagged<Z, B> contraMap(Function<? super Z, ? extends S> fn) {
        return (Tagged<Z, B>) Costrong.super.<Z>contraMap(fn);
    }
}
